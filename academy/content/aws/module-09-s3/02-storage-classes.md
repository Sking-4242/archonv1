...
**Transition timing and billing minimums:** There is no minimum age in S3 Standard before a lifecycle rule can transition an object to Standard-IA or any other class — AWS removed that restriction in 2022. However, each destination class has a **minimum storage duration for billing purposes**: Standard-IA and One Zone-IA charge for at least 30 days; Glacier Instant Retrieval and Flexible Retrieval charge for at least 90 days; Deep Archive charges for at least 180 days. If you delete or transition an object before the minimum billing duration, you are charged as if it stayed for the full minimum. The console may warn about transitions configured with very small day counts as a cost caution, not a technical block.
...
**Trap 3: There is no minimum age before a lifecycle transition to Standard-IA.** AWS removed the 30-day-in-Standard transition prerequisite in 2022. You can configure a lifecycle rule to transition objects to Standard-IA at any age. However, Standard-IA has a 30-day **billing minimum** — if you transition and then delete an object after 5 days in Standard-IA, you are still charged for 30 days. Structuring transitions around billing minimums (not technical minimums) is the correct exam framing.
...
**Trap 5: All storage classes offer the same availability SLA.** They do not. S3 Standard offers 99.99% availability. Standard-IA offers 99.9%. One Zone-IA offers only 99.5% — lower than Standard-IA because it stores data in a single AZ rather than across three. Glacier classes have no formal availability SLA for immediate access because retrieval is intentionally asynchronous. Scenarios involving high availability requirements for frequently served content should always land on S3 Standard. The One Zone-IA vs. Standard-IA availability difference (99.5% vs. 99.9%) is a direct exam test point.
...
- Standard-IA reduces storage cost for infrequently accessed data with 99.9% availability and three-AZ durability; One Zone-IA reduces cost further but accepts single-AZ durability and a lower 99.5% availability SLA — use only for data that can be recreated or that already exists elsewhere.
